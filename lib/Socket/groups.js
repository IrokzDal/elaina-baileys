...
export const makeGroupsSocket = (config) => {
    ...
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            const groups = getBinaryNodeChildren(
                groupsChild,
                'group'
            );
    ...
    return {
        ...
        groupRequestParticipantsUpdate: async (
            ...
            const nodeAction = getBinaryNodeChild(
                node,
                action
            );
        ...
        groupInviteCode: async (jid) => {
            ...
            const inviteNode = getBinaryNodeChild(
                result,
                'invite'
            );
        ...
        groupAcceptInvite: async (code) => {
            ...
            const result = getBinaryNodeChild(
                results,
                'group'
            );
...
export const extractGroupMetadata = (result) => {
    const group = getBinaryNodeChild(
        result,
        'group'
    );

    if (!group) {
        const errorNode = getBinaryNodeChild(
            result,
            'error'
        );
        ...
        if (errorNode) {
            const code = errorNode.attrs.code
                ? +errorNode.attrs.code
                : 500;

            const text =
                errorNode.attrs.text ||
                'group metadata query failed';
    ...
    if (!group.attrs.id) {
        throw new Boom(
            'Invalid group metadata response: missing group id',
            {
                data: group
            }
        );
    ...
    const descChild = getBinaryNodeChild(
        group,
        'description'
    );
    ...
    if (descChild) {
        ...
        descOwner = descChild.attrs.participant
            ? jidNormalizedUser(
                descChild.attrs.participant
            )
            : undefined;

        descOwnerPn = descChild.attrs.participant_pn
            ? jidNormalizedUser(
                descChild.attrs.participant_pn
            )
            : undefined;

        descOwnerUsername =
            descChild.attrs.participant_username ||
            undefined;
    ...
    const metadata = {
        ...
        ownerUsername:
            group.attrs.creator_username ||
            undefined,

        owner_country_code:
            group.attrs.creator_country_code,
        ...
        participants:
            getBinaryNodeChildren(
                ...
                /*
                 * Si WhatsApp proporciona el número real
                 * (@s.whatsapp.net), se utiliza ese.
                 *
                 * Si NO proporciona phone_number,
                 * usamos el @lid como respaldo.
                 */
                const phoneNumber =
                    isLidUser(attrs.jid) &&
