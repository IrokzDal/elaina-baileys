...
const requestPairingCode = async (phoneNumber, customPairingCode) => {
    if (pendingPairing) {
        const secondsLeft = Math.max(
            0,
            Math.ceil((pendingPairing.expiresAt - Date.now()) / 1000)
        )

        throw new Boom(
            'a pairing code is already pending; finish it, wait for it to expire, or call cancelPairingCode()',
            {
                statusCode: 409,
                data: {
                    phoneNumber: pendingPairing.phoneNumber,
                    secondsLeft
                }
            }
        )
    ...
    if (
        ...
        throw new Boom(
            'phone number must be in international format without + or leading 0',
            {
                statusCode: 400,
                data: {
    ...
    // Generar código
    const pairingCode =
        customPairingCode ?? bytesToCrockford(randomBytes(5))
    ...
    const guardMs =
        pairingCodeTimeoutMs + PAIRING_RESPONSE_GRACE_MS
    ...
    const timer = setTimeout(() => {
        logger.info(
            ...
            'pairing code expired without being used'
        )
    ...
    pendingPairing = {
        phoneNumber: normalizedNumber,
        expiresAt: Date.now() + pairingCodeTimeoutMs,
        timer
    }

    // Guardar credenciales del pairing
            ...
            content: [
                {
                    ...
                    content: [
...

                        {
                            ...
                            // MODIFICADO:
                            // Antes: new Uint8Array(1)
                            // Ahora igual que @ikyybella/baileys
                            content: '0'
                        }
                    ]
                }
    ...
    if (!response) {
        ...
        throw new Boom(
            'the server never answered the pairing request',
            {
    ...
    // Verificar que WhatsApp haya registrado el pairing
    if (!pairingRef?.content) {
        clearPendingPairing()

        throw new Boom(
            'the server accepted the pairing request without registering it',
            {
                statusCode: 500,

                data: {
                    response:
                        binaryNodeToString(response)
                }
            }
        )
    }
